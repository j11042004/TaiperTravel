...
struct Keychain<Type: Codable> {
    ...
    var wrappedValue: Type? {
        set {
            do {
                ...
                try keychainItem.saveItem(data)
            }
            catch {
        ...
        get {
            do {
                let saveData = try keychainItem.readItem()
                let result = try JSONDecoder().decode(Type.self, from: saveData)
                return result
            }
            catch {
                return nil
    ...
    //MARK:- Init
    init(key: KeychainKey, accessGroup : String? = nil) {
        keychainItem = .init(key: key, accessGroup: accessGroup)
    }
}
...
struct KeychainItem {
    ...
    init(key: KeychainKey, accessGroup : String? = nil) {
        ...
        self.account = Bundle.main.bundleIdentifier! + key.rawValue
...
extension KeychainItem {
    public static func removeAllKeychain() {
        KeychainKey.allCases.forEach {
            try? KeychainItem(key: $0).deleteItem()
        }
    }
}
